...
class Event extends React.Component {
  ...
  addEvent = event => {
    ...
    axios
      .post("http://localhost:8000/events", {
        ...
        date_start:
          this.state.startDateEvent + " " + this.state.startHourEvent + ":00",
        date_end:
          this.state.endDateEvent + " " + this.state.endHourEvent + ":00",
        family_id: 1
      })
      .then(res => {
        this.setState({ events: [...this.state.events, res.data] }, () =>
          cogoToast.success("Vos dates ont bien été enregistrées")
        );
      });
  };
  ...
  removeEvent = id => {
    axios.delete(`http://localhost:8000/events/${id}`).then(response => {
          ...
          <Modal className="modal_style" isOpen={this.state.showModal}>
            ...
            <form className="flex:column flex-both:center" action="#">
              <div classname="flex:column flex-cross:center space:stack">
                <label className="space:stack" htmlFor="titre">
                  Titre :
                </label>
                <input
              ...
              <div className="flex:column flex-cross:center space:stack">
                <label className="space:stack" htmlFor="start-date">
                  Date de début :
                </label>
                <input
                  className="flex:1 space:inset-squish"
                ...
                <label className="space:stack" htmlFor="start-hour">
                  Heure de début :
                </label>
                <input
                  className="flex:1 space:inset-squish"
                  type="time"
                  name="startHourEvent"
                  value={this.state.startHourEvent}
                  onChange={this.handleInputChange}
                />
              </div>
              <div className="flex:column flex-cross:center space:stack">
                <label className="space:stack" htmlFor="end-date">
                  Date de fin :
                </label>
                <input
                  className="flex:1 space:inset-squish"
                ...
                <label className="space:stack" htmlFor="end-hour">
                  Heure de fin :
                </label>
                <input
                  className="flex:1 space:inset-squish"
              ...
              <div className="button_change_period">
                ...
                <button onClick={this.addEvent} style={{ fontSize: "3vh" }}>
                  Valider
                </button>
              </div>
            </form>
